...
function getSyncWarnings(steps) {
  const warnings = [];
  let hasBudget = false;
  ...
  if (!hasBudget) warnings.push('No budget picker is marked for the frontend quote summary.');
...
function App() {
  ...
  const warnings = useMemo(() => getSyncWarnings(steps), [steps]);
    ...
    <main className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <aside className="fp-panel p-5 md:p-6">
        ...
        <div className="mt-5 space-y-3">
          ...
          )) : <div className="fp-success">Budget, timeline, service picker, and route targets look synced.</div>}
        </div>
